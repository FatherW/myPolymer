...
app.directive('metalCompanyFilter', function ($http, $compile, $templateRequest, $interval, $mdDialog,  $dazzleUser,  $dazzleS3, $dazzlePopup, $ocLazyLoad) {
    var name = 'metalCompanyFilter';
    var link = {
        restrict: 'E',
        scope: true,
        templateUrl: "https://d27btag9kamoke.cloudfront.net/builder6.0/metalCompanyFilter/element.html?id=" + new Date().getTime(),
                ...
                $http(query_init).then(function (result) {
                    ...
                    if (result.data.code < 0) {
                        ...
                        if (number == "hot") {
                            ...
                            $scope.hotData = $scope.hotData.concat(data);
            ...
            $scope.getCompanyQuery = function() {
                ...
                if ($scope.keyword) {
                    $scope.keywordstring = { "bool": { "should": [
                        {"wildcard": {"title": "*"+$scope.keyword+"*"}},
                        {"match_phrase": {"body": $scope.keyword}},
                        {"wildcard": {"company_address": "*"+$scope.keyword+"*"}},
                        {"wildcard": {"company_code": "*"+$scope.keyword+"*"}},
                        {"wildcard": {"company_name_en": "*"+$scope.keyword+"*"}},
                        {"wildcard": {"company_tel": "*"+$scope.keyword+"*"}},
                        {"wildcard": {"company_type": "*"+$scope.keyword+"*"}}
