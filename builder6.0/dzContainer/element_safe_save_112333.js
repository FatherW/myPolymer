...
app.directive('dzContainer', function ($document,$compile, $templateRequest, $mdDialog,$dazzlePopup,$dazzleS3,$dazzleUser,$dazzleData,$dazzleInit,$dazzleFn,userInfo,pageInfo) {
            ...
            return{
                pre : function(scope,element,attribute) {
					...
					pageInfo.init().then(function(){
					    //element.html(html); 
                        $('dz-container').html(pageInfo.model.body);
                        for(var key in pageInfo.model.masterBlock){
                            $('#'+key).attr('dz-master','');
                        }
