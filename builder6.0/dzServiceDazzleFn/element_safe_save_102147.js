       ...
       dazzle.service("$dazzleFn", function ($http,$dazzleS3,$dazzleData,$dazzlePopup,$templateRequest,$dazzleUser,$dazzleInit,$mdDialog,$compile,$rootScope) {
           ...
           var that = this;
			...
			// Only update html by atom data, not db. Will save object in db
			this.updateHtml = function(obj){
                ...
                $dazzleUser.setDazzleInfo('newAtom',atom);
...
//                that.saveElasticAtom(obj.id,obj,false);
			...
			// Used when save button click or value has been changed.            
			...
			// Used when initial load value.
           // Update global atom variable as well.
            this.updateHtml = function(obj){
                console.log('Load Obj',obj.id,obj);
...
//                    console.log(thisPageJson);
                    ...
                    console.log('Init Model',scp.id);

                    that.loadElasticAtom(scp.id,false).then(function(item){
                        scp.model = item;
//                        console.log('Init Model',scp.model);
                        that.useTemplate(scp.model).then(function(){
                            resolve();
                        });
            ...
            this.getElasticAtom = function (id,isMaster) {
                return new Promise(function (resolve, reject) {
           ...
           this.loadElasticAtom = function (id,isMaster) {
               return new Promise(function (resolve, reject) {
                   ...
                   that.getElasticAtom(id,isMaster).then(function(item){
                       var obj = JSON.parse(item.object);
                       if (obj.isDb)
                           that.getDataValue(obj.db).then(function(result){
                               obj.data = result;
                               resolve(obj);
                           });
                       else
                        ...
                        $dazzleData.getAtomData(db).then(function(value){
                           ...
                           $dazzleData.getFieldSchemaByFieldName(db.table,db.field).then(function(schema){
                                ...
                                if (angular.isUndefined(value) || !value) {
                                        resolve("å°æ‡‰Table: "+db.table+". å°æ‡‰Field: "+db.field);
                                }
                                else
                        ...
                        switch(type) {
                            ...
                            case 'password':
                                var confirm = $mdDialog.prompt()
                                    .title('ä½ è¦è®Šæ›´è³‡æ–™å—Ž?')
                                    .textContent('è¼¸å…¥ä½ çš„è³‡æ–™')
