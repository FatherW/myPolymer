console.log('dzEditSchemaPopup');
var app = angular.module('demoApp');
app.directive('dzEditDzschemaPopup', function ($compile, $templateRequest, $mdDialog, $dazzlePopup) {
    var name = 'dzEditDzschemaPopup';
    var link = {
        restrict: 'E',
        scope: true,
...
//                console.log('dazzle-user-' +params.index, 'website/' + params.websiteId + '/content/' + $scope.table + '-table.json');
            ...
            //        console.log('Edit Schema Website',params.websiteId ,$dazzleUser.dazzleInfo['websiteId']);
                ...
                $scope.saveSchema = function () {
                        ...
                        for (var i = 0; i < $scope.schema.length; i++) {
                            if ($scope.schema[i].field && $scope.schema[i].mapping) {
                                $scope.createMapping($scope.table, $scope.schema[i]);
                            }
                ...
                $scope.initSetting = function (s) {
                    s.setting = null;
                    $ocLazyLoad.load('https://d25k6mzsu7mq5l.cloudfront.net/backend6.0/' + s.directive + '/setting.js', {cache: false}).then(function () {
                        s.setting = 'https://d25k6mzsu7mq5l.cloudfront.net/backend6.0/' + s.directive + '/setting.html';
                ...
                $scope.createMapping = function ( type, schema) {
                    return new Promise(function (resolve, reject) {
