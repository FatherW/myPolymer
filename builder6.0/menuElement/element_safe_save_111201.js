...
app.directive('menuElement', function ($ocLazyLoad) {
    var path = "https://d27btag9kamoke.cloudfront.net/builder6.0/menuElement/";
    var menuElement = {
        restrict: 'E',
        priority: 1000,
        scope: true,
...
angular.module('demoApp').controller('userGalleryPopupController', function ($scope, $window, $uibModalInstance) {
	...
	var awsKey = store.get('key');
	var website = store.get('website');
    AWS.config = new AWS.Config();
	AWS.config.accessKeyId = awsKey.AccessKeyId;
	AWS.config.secretAccessKey = awsKey.SecretAccessKey;
	AWS.config.sessionToken = awsKey.SessionToken;
	AWS.config.region = 'ap-northeast-1';
	AWS.config.httpOptions.xhrAsync = false;
