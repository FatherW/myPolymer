...
app.directive('exportPopup', function ($compile, $templateRequest, $mdDialog, $uibModal) {
    ...
    var link = {
        restrict: 'E',
        scope: true,
        templateUrl: "https://d25k6mzsu7mq5l.cloudfront.net/builder6.0/exportPopup/element.html?id=" + new Date().getTime(),
...
//                                    $dazzleS3.copyFile('dazzle-user-' + params.$scope.$dazzleUser.getUser().uid + '/website/' + params.$scope.$dazzleUser.dazzleInfo['websiteId'] + '/json/' + template + '.json', 'dazzle-user-' + params.$scope.$dazzleUser.getUser().uid, 'website/' + params.$scope.$dazzleUser.dazzleInfo['websiteId'] + '/json/' + page + '.json'),
