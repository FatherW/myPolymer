...
//                console.log('dazzle-user-' +params.index, 'website/' + params.websiteId + '/content/' + $scope.table + '-table.json');
            ...
            //        console.log('Edit Schema Website',params.websiteId ,$dazzleUser.dazzleInfo['websiteId']);
                ...
                $scope.loadDirectives = function () {
                    return new Promise(function (resolve, rejcet) {
                        ...
                        if ($scope.isForm) {
                            ...
                            $dazzleS3.getFile('dazzle-template','file6.0/data-schema-tpl.json').then(function(json){
                ...
                $scope.saveSchema = function () {
                    if ($scope.tableJson && $scope.tableJson.data.type && $scope.tableJson.data.type == 'dynamodb' && $scope.tableJson.data.index) {
                        for (var i = 0; i < $scope.schema.length; i++) {
                            if ($scope.schema[i].field && $scope.schema[i].mapping) {
                                $scope.createMapping($scope.tableJson.data.index, $scope.tableJson.data.table, $scope.schema[i]);
                            }
            				...
            				// 		$dazzleData.saveTableByName($dazzleUser.dazzleInfo['thisTable'],table).then(function(result) {
                ...
                $scope.createMapping = function (index, type, schema) {
                    return new Promise(function (resolve, reject) {
                        ...
                        body[type]['properties'][field] = mapping;
