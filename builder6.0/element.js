...
app.directive('newEditorHeader', function ($compile, $timeout, $uibModal, $mdDialog, $mdToast, $dazzleS3, $dazzlePopup) {
    var newEditorHeader = {
        restrict: 'E',
        priority: 1000,
        scope: true,
        templateUrl: "http://d25k6mzsu7mq5l.cloudfront.net/builder6.0/newEditorHeader/element.html?id=" + new Date().getTime(),
                    ...
                    $dazzleS3.saveFile($scope.userBucket,$scope.websiteKey + 'page/'+$scope.thisPage+'/full.html',$scope.fullHtml).then(function(result){
                            ...
                            $http({
                                ...
                                "data": {
                                    ...
                                    "website": {
                                        "website": "我的網站1504771045885",
                                        "bucket": "benluk.dazzle.website",
                                        "subdomain": "benluk",
            ...
            $scope.saveRootHtml = function () {
                return new Promise(function (resolve, reject) {
                    ...
                    if ($scope.templateMode) {
                        ...
                        //rootHtml.find('editor-header').remove();
                        //rootHtml.find('editor-body').remove();
                        //rootHtml.find('[md-theme-style]').remove();
                        //rootHtml.find('[data-requiremodule]').remove();
                        //console.log(rootHtml.html());
                        $dazzleS3.saveFile($scope.exportBucket, 'admin' + '/' + $scope.thisPage + '.html', rootHtml
                            .replace(/(\sng-\w+-\w+="(.|\n)*?"|\sng-\w+="(.|\n)*?"|\sng-(\w+-\w+)|\sng-(\w+))/g, "")
                            .replace(/<!--(.*?)-->/gm, ""));*/
                        resolve();
                    } else {
