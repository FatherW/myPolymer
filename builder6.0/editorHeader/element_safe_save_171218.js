            ...
            $scope.saveRootHtml = function () {
                return new Promise(function (resolve, reject) {
                    ...
                    if ($scope.templateMode) {
                        console.log($scope.templateMode);
                        console.log($scope.exportBucket);
                        $dazzleS3.saveFile($scope.exportBucket, 'admin' + '/' + $scope.thisPage + '.html', document.documentElement.outerHTML);
                        resolve();
                    } else {
