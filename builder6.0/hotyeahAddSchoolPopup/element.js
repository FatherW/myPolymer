...
app.directive('hotyeahAddSchoolPopup', function ($compile, $templateRequest, $mdDialog, $uibModal) {
    ...
    var link = {
        restrict: 'E',
        scope: true,
        templateUrl: "https://d25k6mzsu7mq5l.cloudfront.net/builder6.0/hotyeahAddSchoolPopup/element.html?id=" + new Date().getTime(),
...
//                $scope.thisPageJson=$dazzleUser.getDazzleInfo('thisPageJson');
						...
						if (id){
                            ...
                            $scope.loadSchool($scope.id).then(function(result){
                               ...
                               // $scope.$apply = function() {
                                	console.log(result);
                                    $scope.model = result;
                                    // if (!angular.isUndefined($scope.model['Albums']))
	                                 //    $scope.noPhotos = $scope.model['Albums'].length;
                                    // else
                                     //    $scope.noPhotos = 0;
                                    //
									// if (!angular.isUndefined($scope.model['Files']))
									// 	$scope.noFiles = $scope.model['Files'].length;
									// else
									// 	$scope.noFiles = 0;
                                    //
									// if (!angular.isUndefined($scope.model['課程s']))
									// 	$scope.noCourses = $scope.model['課程s'].length;
									// else
									// 	$scope.noCourses = 0;
				...
				$scope.generate = function() {
                    ...
                    if (!$scope.isEdit) {
                        ...
                        $dazzleData.updateElasticRecord('hotyeah','school',$scope.model['id'],$scope.model).then(function(result) {
                            console.log(result);
                        });
					}
