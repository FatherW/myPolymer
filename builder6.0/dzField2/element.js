...
app.directive('dzField2', function ($compile, $templateRequest, $http, $mdDialog,$dazzleUser,$dazzleS3,$dazzleInit,$dazzlePopup,$dazzleData,$dazzleFn) {
    var dzField2 = {
        ...
        //                 console.log("It's a dzField double click");
        //             });
        //         }
        //     }
        // },
        ...
        link: function (scope, element, attrs) {
            ...
            $dazzleFn.editorCustomInit(scope,element, attrs).then(function (object) {
                    ...
                    $dazzleFn.initRenderByType(scope.model,element);
                    $compile(element.contents())(scope);
...
//                    $element.dblclick();
            ...
            $scope.update = function(){
    			var html = $dazzleFn.renderByType($scope,$element);
                $dazzleUser.dazzleInfo['newAtom'][$scope.model.id] =$scope.model;
                $element.html(html);
                $compile($element.contents())($scope);
                console.log('Update Model',$scope.model);
                $dazzleFn.saveElasticAtom($scope.model.id,$scope.model,false);
                
            }
            $scope.cancel = function() {
                	var type = $element.attr('type') || 'text';
					var html = $dazzleFn.renderByType($scope,type);
					$element.html(html);						
            }
                    ...
                    $dazzleFn.saveElasticAtom($scope.model.id,$scope.model,false).then(function(){
                        ...
                        $scope.menuOptions = [
                            ...
                            ["更新資料",function() {
                                ...
                                console.log($scope.model);
                            	var type = $element.attr('type') || 'text';
                                $dazzleFn.updateInputByType($scope,$element);
