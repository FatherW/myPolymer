...
app.directive('kpleeSuntaGallery', function($compile, $templateRequest, $mdDialog) {
    var kpleeSuntaGallery = {
        ...
        controller: function($scope, $element, $attrs, $http, $sce, $timeout, $linq, $mdDialog) {
			...
			function checkLang(rowdate){
    	var thisLang = store.get('thislang') || 'zh';
    	if (thisLang == 'eng') {
            for (var i = 0; i < rowdate.length; i++) {
                rowdate[i]["Cname2"] = rowdate[i]["Cname1"];
                rowdate[i]["Pname2"] = rowdate[i]["Pname1"];
            }
        } 
    	else if (thisLang == 'cn') {
            for (var i = 0; i < rowdate.length; i++) {
                rowdate[i]["Cname2"] = rowdate[i]["Cname3"];
                rowdate[i]["Pname2"] = rowdate[i]["Pname3"];
            }
        } 
        else {
            $scope.lang = 'zh';
