...
app.directive('editor-news-list', function ($compile, $templateRequest) {
    var http = "https://d25k6mzsu7mq5l.cloudfront.net/";
    var path = "builder6.0/editor-news-list/";
    var directiveName = "editor-news-list";
    var editor-news-list = {
        restrict: 'E',
        priority: 1000,
        scope: true,
        link: function (scope, element) {
            //console.log('editor-news-list loading start');
            $templateRequest(http + path + "html/element.html?id=" + new Date().getTime()).then(function (html) {
                var template = angular.element(html);
                element.html(template);
                $compile(template)(scope);
            });
            //console.log('editor-news-list loading end');
        },
        controller: function ($scope, $element, $attrs) {
            //console.log('editor-news-list init start');
            $scope.id = $element.attr('id') || "ele" + new Date().getTime() + "-" + Object.keys($scope.atom).length;
            $element.attr('id', $scope.id);
            if (angular.isUndefined($scope.atom[$scope.id])) {
                $scope.atom[$scope.id] = {
                    "id": $scope.id,
                    "type": "editor-editor-news-list-element"
                };
                if (!$.trim($element.html())) {
                    $scope.atom[$scope.id].html = '<div>editor-news-list</div>'
                } else {
            ...
            //console.log('editor-news-list init end');
