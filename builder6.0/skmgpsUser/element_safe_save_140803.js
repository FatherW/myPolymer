...
app.directive('dazzleUser', function ($compile, $templateRequest,  $mdDialog,$dazzlePopup,$dazzleUser,$http,$dazzleElastic) {
    var dzUser = {
        ...
        priority: 1000,
        scope: true,
        templateUrl: "//d27btag9kamoke.cloudfront.net/builder6.0/dazzleUser/element.html?id=" + new Date().getTime(),
        ...
        controller: function ($scope, $element, $attrs) {
            ...
            $scope.loginPopup = function () {
                $dazzlePopup.login().then(function (user) {
                    ...
                    $scope.$apply(function () {
                        $scope.cuser = $dazzleUser.getUser();
                    });
                    $scope.goToDashboard();
                });
            };
            ...
            $scope.getMyWebsite = function () {
                $scope.myWebsites = {};
                if (angular.isArray($scope.cuser.webdomain)) {
                    for (var i = 0; i < $scope.cuser.webdomain.length; i++) {
                        getWebsiteJson($scope.cuser.webdomain[i]);
                    }
                } else {
                    getWebsiteJson($scope.cuser.webdomain);
                }

                function getWebsiteJson(websiteId) {
                    $dazzleS3.getJson("dazzle-user-" + $scope.cuser.uid, "website/" + websiteId + '/json/website.json').then(function (json) {
                        $scope.$apply(function () {
            ...
            $scope.editWebsite = function (website) {
                window.open(
                    "http://builder.dazzle.website/index.html?token:===:" + $scope.cuser.token + "&&&websiteId:===:" + website.website + "&&&editPage:===:" + "index",
                    '_blank'
                );
            }
        }
