...
app.directive(name, function ($compile, $templateRequest, $mdDialog, $uibModal,$dazzleUser,$dazzleS3,$dazzlePopup,$dazzleFn,$dazzleInit) {
    var link = {
        restrict: 'E',
        scope: true,
        templateUrl: "https://d25k6mzsu7mq5l.cloudfront.net/builder6.0/recoveryPopup/element.html?id=" + new Date().getTime(),
                    ...
                    if (Object.prototype.toString.call(date) === "[object Date]") {
                        if (isNaN(date.getTime())) {
                            ...
                            if (date.getTime() > $scope.today.getTime() || date.getTime() < $scope.minDate.getTime()) {
                                ...
                                var confirm = $mdDialog.confirm()
                                    .title('是否把本頁還原到')
                                    .textContent(date + "<p>注意：所有已儲存但未匯出的檔案將會移除</p>")
                                    .ariaLabel('回復時間')
                                    .ok('是')
                                    .cancel('否');
                                $mdDialog.show(confirm).then(function () {
