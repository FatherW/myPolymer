            ...
            $scope.menuOptions = [
                ...
                ['表格設置', function ($itemScope, node) {
                        var modalInstance = $uibModal.open({
                            animation: true,
                            keyboard: false,
                            templateUrl: 'views/formTablePopup.html' + "?id=" + new Date().getTime(),
                            controller: 'formTablePopupController',
                            size: 'lg',
                            resolve: {
                                "table": function () {
                                    if (angular.isUndefined($scope.atom[id].formTable)) {
                                        $scope.atom[id].formTable = {};
                                    }
                                    return $scope.atom[id].formTable
                                }
                            }
                        });
                        modalInstance.result.then(function (table) {
                            $scope.atom[id].formTable = table;
                            $scope.updateHtml();
                        });
                    }]
                    /*,
