...
app.directive('yesicanMenu', function ($compile, $templateRequest) {
    var http = "https://d27btag9kamoke.cloudfront.net/";
    var path = "builder6.0/yesicanMenu/";
    var directiveName = "yesicanMenu";
    var yesicanMenu = {
        restrict: 'E',
        priority: 1000,
        scope: true,
        link: function (scope, element) {
            //console.log('yesicanMenu loading start');
            $templateRequest(http + path + "html/element.html?id=" + new Date().getTime()).then(function (html) {
                var template = angular.element(html);
                element.html(template);
                $compile(template)(scope);
            });
            //console.log('yesicanMenu loading end');
        },
        controller: function ($scope, $element, $attrs) {
            //console.log('yesicanMenu init start');
            $scope.id = $element.attr('id') || "ele" + new Date().getTime() + "-" + Object.keys($scope.atom).length;
            $element.attr('id', $scope.id);
            if (angular.isUndefined($scope.atom[$scope.id])) {
                $scope.atom[$scope.id] = {
                    "id": $scope.id,
                    "type": "editor-yesicanMenu-element"
                };
                if (!$.trim($element.html())) {
                    $scope.atom[$scope.id].html = '<div>yesicanMenu</div>'
                } else {
            ...
            //console.log('yesicanMenu init end');
