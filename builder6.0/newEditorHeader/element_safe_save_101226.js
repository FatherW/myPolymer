...
//                }
            ...
            $scope.changePage = function (selectedPage) {
                if (selectedPage !== $scope.thisPage) {
                    ...
                    location.href = "http://builder.dazzle.website/page.html?singlePage:===:" + thisPage + "&&&uid:===:" + $dazzleUser.getUser().uid + "&&&websiteId:===:" + $dazzleUser.getDazzleInfo('websiteId') + "&&&editPage:===:" + thisPage + "&&&token:===:" + $dazzleUser.getUser().token
...
//                    store.set('thislang', selectedLang);
            ...
            $scope.createPageByTemplate = function (newPageName) {
                ...
                $dazzleS3.saveJson(userBucket, websiteKey + 'json/page.json', pageJson).then(function () {
                    copyed++;
                    if (copyed >= 4) {
                        copyDone();
                    }
