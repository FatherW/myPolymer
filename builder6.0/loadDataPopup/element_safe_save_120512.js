...
app.directive('loadDataPopup', function ($compile, $templateRequest, $mdDialog, $uibModal) {
    var name = 'loadDataPopup';
    var link = {
        restrict: 'E',
        scope: true,
        templateUrl: "https://d27btag9kamoke.cloudfront.net/builder6.0/loadDataPopup/element.html?id=" + new Date().getTime(),
        link: function (scope, element, attrs) {
            scope.http = "https://d27btag9kamoke.cloudfront.net/";
            scope.directiveId = "loadDataPopup";
            scope.type = "loadDataPopup";
            scope.templatePath = "builder6.0/" + scope.directiveId + "/element.html?id=" + new Date().getTime()
            scope.templateUrl = scope.http + scope.templatePath;
        },        
        controller: function ($scope,$window,  $element, $mdToast,$ocLazyLoad,$mdDateLocale, $attrs,$dazzleS3, $dazzlePopup,$http, $mdDialog, $mdSidenav, $mdBottomSheet,$dazzleUser,$dazzleFn,$dazzleData,$dazzleInit,moment) {
			...
			$scope.isForm = params.isForm || false;
			$scope.editable = true;
...
//					console.log('Get','dazzle-user-' + $dazzleUser.getUser().uid+'/website/' +$dazzleUser.dazzleInfo['websiteId'] + '/' + "content/" + $scope.table + "-table.json");
                    var myTable;
                    if ($scope.isForm)
                        myTable = 'website/' +$dazzleUser.dazzleInfo['websiteId'] + '/' + "content/" + $scope.table + "-form.json";
                    else
                        myTable = 'website/' +$dazzleUser.dazzleInfo['websiteId'] + '/' + "content/" + $scope.table + "-table.json"
				console.log('My Form',myTable);
...
//				    if (!$scope.isForm) {
				    ...
				    // } else {
				    //     $scope.thisSchema = [];
				    //     angular.forEach($scope.tableJson.schema,function(item,key){
				    //        item.directive.headerName = item.headerName;
				    //        $scope.thisSchema.push(item.directive);
				    //     });
				    //    resolve(JSON.parse(angular.toJson($scope.thisSchema)));
				    // }
				});
			};
...
//				alasql('SELECT * INTO XLSX("' + $scope.table + '.xlsx",{headers:true}) FROM ?', [rowData]);
			...
			$scope.clean = function (obj) {
                ...
                $scope.close = function() {
                    ...
                    $dazzleUser.dazzleInfo['thisTable'] = params.parentTable;
                    $mdDialog.hide();
                }
