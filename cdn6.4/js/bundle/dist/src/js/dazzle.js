...
define(
    ['jquery', 'angular', 'aws-sdk', 'store', 'ace',  'medium-editor', 'ag-grid-enterprise', 'alasql', 'xlsx', 'moment', 'jquery-ui', 'bootstrap', 'slick', 'angular-animate', 'angular-aria', 'angular-route', 'angular-messages', 'angular-ui-bootstrap', 'angular-ui-sortable', 'angular-ui-ace', 'angular-ui-tree', 'angular-material', 'angular-material-timepicker', 'angular-material-fileinput', 'angular-slick', 'angular-grid','angular-bootstrap-calendar', 'ocLazyLoad', 'angular-contextMenu', 'multirange-slider', 'angular-moment', 'hotkeys','angular-material-sidemenu'],
    function ($, angular, AWS, store, ace,  MediumEditor, agGrid, alasql, xlsx, moment, hotkeys) {
        ...
        dazzle.service('$dazzleUser', function ($http, $location,$interval, hotkeys) {
            ...
            this.setDazzleInfo = function (key, value) {
                dazzleInfo[key] = value;
                console.log('Dazzle Info', dazzleInfo);
            }
            this.getDazzleInfo = function (key) {
                console.log('Dazzle Info', dazzleInfo);
                return dazzleInfo[key];
            }
...
//                    scope.initStep2();
			...
			scope.loadDirectiveInfo = function() {
                ...
                // if (!angular.isUndefined($dazzleUser.getDazzleInfo('customDirectivesJson')))
                //     return;
...
//                });
            ...
            scope.loadPageInfo = function () {
                // if ($dazzleUser.getDazzleInfo('thisPageJson'))
                //     return;
                // if ($dazzleUser.getDazzleInfo('masterJson'))
                //     return;
                // if ($dazzleUser.getDazzleInfo('pageJson'))
                //     return;
  ...
  //              return new Promise(function (resolve, reject) {
      ...
      //                  resolve();
                        ...
                        // $ocLazyLoad.load([
                        //     'https://' + exportBucket + '/js/' + thisPage + '.js',
                        //     'https://' + exportBucket + '/css/' + thisPage + '.css',
                        //     'https://' + exportBucket + '/js/master.js',
                        //     'https://' + exportBucket + '/css/master.css',
   ...
   //             });
			...
			scope.loadAtomInfo = function () {
                // if ($dazzleUser.getDazzleInfo('atom'))
                //     return;
                // if ($dazzleUser.getDazzleInfo('masterAtom'))
                //     return;
...
//            }
            ...
            scope.saveAtom = function() {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    console.log('Save Atom',scope.userBucket, scope.websiteKey + 'page/' + scope.thisPage + '/atom.json', JSON.parse(angular.toJson(atom)));
                    $dazzleS3.saveJson(scope.userBucket, scope.websiteKey + 'page/' + scope.thisPage + '/atom.json', JSON.parse(angular.toJson(atom))).then(function () {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    scope.useTemplate = function () {
                        return new Promise(function (rs, rj) {
                            ...
                            scope.updateHtml(scope).then(function () {
                                rs();
                            });
                        });
                    }
            ...
            scope.editRootHtml = function () {
                ...
                $dazzlePopup.code($('#root').html(), 'html').then(function (newCode) {
                    ...
                    rootHtml.find("[custom]").each(function (index, element) {
                        var id = $(element).attr('id');
                        if (!angular.isUndefined(scp.atom[id])) {
                    ...
                    rootHtml.find("[master]").each(function (index, element) {
                        var id = $(element).attr('id');
                        if (!angular.isUndefined(scp.masterAtom[id])) {
                            scp.masterAtom[id].html = $(element).html();
                        }
                    });

                    scp.$apply(function () {
                        scp.rootHtml = rootHtml.html();
                    });
...
//                        parent:angular.element('#data-management'),
          ...
          dazzle.service("$dazzleFn", function ($dazzleS3,$dazzleData,$dazzlePopup,$dazzleUser,$dazzleInit,$mdDialog) {
            var that = this;
...
//                        console.log('Init Error');
                        ...
                        $dazzleData.getFieldSchemaByFieldName(db.table,db.field).then(function(schema){
                            ...
                            switch(schema.directive){
                                ...
                                case 'video':
                                    value = "https://www.youtube.com/embed/L0MK7qz13bU";
                                break;
