...
//                target.attr('id',id);
        ...
        dzFn.loadMyDirectives = function () {
            var url = "https://d25k6mzsu7mq5l.cloudfront.net/builder6.0/";
            var js,css;
            return new Promise(function (resolve, reject) {
...
//                    $dazzleS3.getJson(userInfo.exportBucket, userInfo.dataKey + file).then(function (result) {
        ...
        dzS3.getMaster = function (file) {
            ...
            return new Promise(function (resolve, reject) {
                if (ext==='json'){
                    $dazzleS3.getJson(userInfo.exportBucket,userInfo.masterKey+file).then(function(result){
                        resolve(result);
                    },function(err){
                        reject(err);
                    });
                } else {
                    $dazzleS3.getFile(userInfo.exportBucket,userInfo.masterKey+file).then(function(result){
                        resolve(result);
                    },function(err){
                        reject(err);
                    });
    ...
    $provide.factory('atomInfo',function($dazzleS3,$dazzleElastic,userInfo,dbFactory) {
        var atomInfo ={};
        ...
        // atomInfo.getMasterAtom = function(id){
        //     return new Promise(function(resolve,reject) {
        //         $dazzleS3.getJson(userInfo.exportBucket, 'json/master/' + id + '-atom.json').then(function (result) {
        //             resolve(result);
        //         }, function (err) {
...
//                 });
        ...
        pageInfo.save = function() {
            ...
            return new Promise(function (resolve, reject) {
                ...
                Promise.all([pageInfo.saveMaster(), pageInfo.saveBody(),pageInfo.saveInfo(),pageInfo.saveUser(),pageInfo.saveJson(),pageInfo.saveBodyScript()]).then(function () {
...
'pageInfo',
    function ($scope, $http, $element, $compile, $templateRequest, $interval, $mdDialog,  $dazzleUser, $dazzleInit, $dazzleS3, $dazzlePopup, $ocLazyLoad,$dazzleData,$dazzleElastic,$dazzleFn,atomInfo,dbFactory,hotkeys,userInfo,dzFn,pageInfo) {
...
//                    location.reload();
        ...
        $scope.adminLogin = function() {
            ...
            if (!isToken) {
                ...
                $dazzleUser.userLogin(user.token).then(function(){
            ...
            // Promise.all([$scope.loadUserInfo(),$scope.loadDirectiveInfo(),pageInfo.loadMaster(),pageInfo.loadAtom()]).then(function () {
        ...
        $scope.loadDirectiveInfo = function() {
            ...
            var owner;
            var user = store.get('user') || null;
            if ($scope.editMode =="admin" && user){
                owner = ["dazzle6.4",user.exportBucket];
            } else {
                owner = ["client-core",location.hostname];
            }
            console.log('My Directive Owner',owner);
