...
console.log('Dazzle Factory 6.5.5 Begin');
var wholeJson = {
  "structure":{"head":{},"body":{}},
  "title":"",
  "meta":[],
        ...
        pageInfo.saveAtom = function() {
            return new Promise(function(resolve,reject){
                for (key in atomInfo.atom){
					...
					pageInfo.model[key]['html'] = $('#'+key).html();
                }
...
//                console.log('My Model',pageInfo.model);
                ...
                for(k in pageInfo.model['head-attrs']){
                    __$('head').attr(k,pageInfo.model['head-attrs'][k]);
                }

                for(k in pageInfo.model['body-attrs']){
                    __$('body').attr(k,pageInfo.model['body-attrs'][k]);
                }
                ...
                var html = __$.html();
                dzS3.saveFile(userInfo.exportBucket,'page.html',html).then(function () {
                    console.log('成功生成');
                    resolve();
                },function(err){
                    console.log('儲存失敗',err);
        ...
        pageInfo.save = function() {
            ...
            return new Promise(function (resolve, reject) {
                ...
                Promise.all([pageInfo.saveMaster(), pageInfo.saveBody(),pageInfo.saveStructure(),pageInfo.saveJson(),pageInfo.saveBodyScript()]).then(function () {
...
//                mydbElastic.= $dazzleUser.getRootdbElastic.);
    ...
    $provide.factory('dbFactory',function($dazzleUser,$dazzleElastic,$dazzlePopup,$ocLazyLoad,$http,userInfo,dzS3){
        ...
        dbFactory.schema = {};
            ...
            $.getJSON("/json/data.json").done(function (json) {
                        dbFactory.data = json;
                }).fail(function () { 
                        dbFactory.data = {};
                });
...
//        var password = _MYPASSWORD || "phy3math4";
        ...
        $scope.adminLogin = function() {
            ...
            $('body').wrapInner( '<dz-container context-menu="dzOptions($event)"></dz-container>');
...
//                  reject();         
        ...
        $scope.loadDirectiveInfo = function() {
            ...
            console.log('Load Directive Info');
            var url = "//d27btag9kamoke.cloudfront.net/builder6.0/";
            var user_bucket = location.hostname;
            return new Promise(function (resolve, reject) {
        ...
        $scope.normalLogin = function() {
            ...
            Promise.all([$scope.loadDirectiveInfo()]).then(function () {
