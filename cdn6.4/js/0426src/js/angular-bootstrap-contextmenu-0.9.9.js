...
.directive('contextMenu', ["$parse", "$q", "CustomService", "$sce", function ($parse, $q, custom, $sce) {
    ...
    function renderContextMenu (params) {
        ...
        console.log('Length',$(document).find('md-dialog').length);
        if($(document).find('md-dialog').length)
            $(document).find('md-dialog').append($ul);
        else
...
//          top: params.event.pageY + 'px',
          left: params.event.screenX+'px',
          top: params.event.screenY+'px',
          "z-index": 10000
      });
